...
class ExecutionAgent:
    ...
    def __init__(self):
        ...
        if STRUCTURED_LOGGING_AVAILABLE:
            ...
            self.logger.info(f"ExecutionAgent initialized - Tools: {TOOLS_AVAILABLE}, Structured Logging: {STRUCTURED_LOGGING_AVAILABLE}")
    ...
    def _initialize_tools(self) -> Dict[str, Any]:
        ...
        if TOOLS_AVAILABLE and get_tool_registry:
            try:
                ...
                # For testing purposes, continue without tools instead of failing
                self.logger.warning("Continuing without tools - execution will be limited")
                tools = {}
        else:
            self.logger.warning("Tool registry not available - execution will be limited")
            tools = {}
    ...
    def _extract_content_from_steps(self, steps: List[Dict[str, Any]]) -> Optional[str]:
        ...
        """
        print(f"🔧 DEBUG: _extract_content_from_steps called with {len(steps)} steps")
        
        for i, step in enumerate(steps):
            print(f"🔧 DEBUG: Checking step {i}: {step}")
            args = step.get('args', {})
            print(f"🔧 DEBUG: Step {i} args: {args}")
            
            content = args.get('content')
            if content:
                print(f"✅ DEBUG: Found content in step {step.get('task_id', 'unknown')} (length: {len(content)} chars)")
                print(f"🔧 DEBUG: Content preview: {content[:200]}...")
                self.logger.info(f"Found content in step {step.get('task_id', 'unknown')} (length: {len(content)} chars)")
                return content
            else:
                print(f"🔧 DEBUG: No content in step {i}")
        
        print(f"❌ DEBUG: No content found in any plan steps")
    ...
    async def execute_plan_async(self, plan: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        print(f"🔧 DEBUG: Starting execute_plan_async with plan: {plan}")
        print(f"🔧 DEBUG: Execution ID: {execution_id}")
        print(f"🔧 DEBUG: STRUCTURED_LOGGING_AVAILABLE: {STRUCTURED_LOGGING_AVAILABLE}")
        
        if STRUCTURED_LOGGING_AVAILABLE:
            try:
                self.react_logger = get_react_logger("executor", execution_id)
                print(f"✅ DEBUG: React logger initialized successfully")
                ...
                print(f"✅ DEBUG: Initial reasoning logged")
            except Exception as e:
                print(f"❌ DEBUG: React logger initialization failed: {e}")
                import traceback
                traceback.print_exc()
        ...
        try:
            ...
            print(f"🔧 DEBUG: Plan details - ID: {plan_id}, Steps count: {len(steps)}")
            print(f"🔧 DEBUG: Steps content: {steps}")
            ...
            if STRUCTURED_LOGGING_AVAILABLE:
                ...
                self.logger.info(f"Plan execution started - Plan ID: {plan_id}, Steps: {len(steps)}, Execution ID: {execution_id}")
            ...
            if STRUCTURED_LOGGING_AVAILABLE:
                try:
                    ...
                    print(f"✅ DEBUG: Reasoning phase logged successfully")
                except Exception as e:
                    print(f"❌ DEBUG: Reasoning logging failed: {e}")
            
            # ACTION: Extract content from plan steps
            if STRUCTURED_LOGGING_AVAILABLE:
                try:
                    self.react_logger.action("extract_content_from_steps", {"steps": len(steps)})
                    print(f"✅ DEBUG: Action phase logged successfully")
                except Exception as e:
                    print(f"❌ DEBUG: Action logging failed: {e}")
            
            print(f"🔧 DEBUG: About to extract content from {len(steps)} steps")
            content = self._extract_content_from_steps(steps)
            print(f"🔧 DEBUG: Content extraction result - Length: {len(content) if content else 0}")
            ...
            if not content:
                ...
                if STRUCTURED_LOGGING_AVAILABLE:
                    try:
                        ...
                        print(f"✅ DEBUG: Error observation logged successfully")
                    except Exception as e:
                        print(f"❌ DEBUG: Error observation logging failed: {e}")
                ...
                print(f"✅ DEBUG: Content extracted successfully - {len(content)} characters")
                if STRUCTURED_LOGGING_AVAILABLE:
                    try:
                        ...
                        print(f"✅ DEBUG: Success observation logged successfully")
                    except Exception as e:
                        print(f"❌ DEBUG: Success observation logging failed: {e}")
            
            # PLANNING: Execute steps sequentially with dependency management
            print(f"🔧 DEBUG: Starting planning phase for {len(steps)} steps")
            if STRUCTURED_LOGGING_AVAILABLE:
                try:
                    ...
                    print(f"✅ DEBUG: Planning phase logged successfully")
                except Exception as e:
                    print(f"❌ DEBUG: Planning logging failed: {e}")
            
            # Execute each step
            step_results = []
            completed_tasks = set()
            
            print(f"🔧 DEBUG: Starting step execution loop")
            for i, step in enumerate(steps):
                step_id = step.get("task_id", f"step_{i}")
                print(f"🔧 DEBUG: Processing step {i+1}/{len(steps)} - ID: {step_id}")
                print(f"🔧 DEBUG: Step details: {step}")
                ...
                if STRUCTURED_LOGGING_AVAILABLE:
                    try:
                        ...
                        print(f"✅ DEBUG: Dependency action logged for step {step_id}")
                    except Exception as e:
                        print(f"❌ DEBUG: Dependency action logging failed for step {step_id}: {e}")
                
                print(f"🔧 DEBUG: Waiting for dependencies for step {step_id}")
                await self._wait_for_dependencies(step, completed_tasks)
                print(f"✅ DEBUG: Dependencies satisfied for step {step_id}")
                ...
                if STRUCTURED_LOGGING_AVAILABLE:
                    try:
                        ...
                        print(f"✅ DEBUG: Step action logged for step {step_id}")
                    except Exception as e:
                        print(f"❌ DEBUG: Step action logging failed for step {step_id}: {e}")
                
                print(f"🔧 DEBUG: Executing step {step_id} with tool {step.get('tool')}")
                step_result = await self._execute_single_step(step, content)
                print(f"🔧 DEBUG: Step {step_id} execution result: {step_result}")
                ...
                print(f"🔧 DEBUG: Step {step_id} success status: {step_success}")
                if STRUCTURED_LOGGING_AVAILABLE:
                    try:
                        self.react_logger.observation(
                            {
                        ...
                        print(f"✅ DEBUG: Step observation logged for step {step_id}")
                    except Exception as e:
                        print(f"❌ DEBUG: Step observation logging failed for step {step_id}: {e}")
                ...
                if STRUCTURED_LOGGING_AVAILABLE:
                    ...
                    self.logger.info(f"Step execution completed - Step: {step_id}, Tool: {step.get('tool')}, Status: {step_result.get('status')}")
            
            # OBSERVATION: Plan execution completed
            print(f"🔧 DEBUG: All steps completed, preparing final observation")
            if STRUCTURED_LOGGING_AVAILABLE:
                try:
                    ...
                    print(f"✅ DEBUG: Final observation logged successfully")
                except Exception as e:
                    print(f"❌ DEBUG: Final observation logging failed: {e}")
            
            successful_steps = sum(1 for r in step_results if r.get('status') == 'completed')
            print(f"🎉 DEBUG: Execution summary - Total: {len(steps)}, Successful: {successful_steps}")
            print(f"🔧 DEBUG: Final step results: {step_results}")
            ...
            if STRUCTURED_LOGGING_AVAILABLE:
                ...
                self.logger.info(f"Plan execution completed - Plan: {plan_id}, Total: {len(steps)}, Successful: {successful_steps}")
            
            return step_results
            
        except Exception as e:
            print(f"❌ DEBUG: Exception in execute_plan_async: {e}")
            import traceback
            traceback.print_exc()
            ...
            if STRUCTURED_LOGGING_AVAILABLE and self.react_logger:
                try:
                    ...
                    print(f"✅ DEBUG: Error logged to react logger")
                except Exception as le:
                    print(f"❌ DEBUG: Error logging to react logger failed: {le}")
    ...
    async def _execute_single_step(self, step: Dict[str, Any], content: str) -> Dict[str, Any]:
        ...
        print(f"🔧 DEBUG: _execute_single_step called")
        print(f"🔧 DEBUG: Task ID: {task_id}")
        print(f"🔧 DEBUG: Tool: {tool}")
        print(f"🔧 DEBUG: Args: {args}")
        print(f"🔧 DEBUG: Doc URI: {doc_uri}")
        print(f"🔧 DEBUG: Content length: {len(content) if content else 0}")
        print(f"🔧 DEBUG: Available tools: {list(self.tools.keys()) if self.tools else 'None'}")
        ...
        try:
            ...
            print(f"✅ DEBUG: Step execution successful: {success_result}")
            return success_result
            
        except Exception as e:
            print(f"❌ DEBUG: Step execution failed: {e}")
            import traceback
            traceback.print_exc()
    ...
    async def _execute_vector_ingestion_with_content(self, doc_uri: str, content: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        print(f"🔧 DEBUG: _execute_vector_ingestion_with_content called")
        print(f"🔧 DEBUG: Doc URI: {doc_uri}")
        print(f"🔧 DEBUG: Content length: {len(content) if content else 0}")
        print(f"🔧 DEBUG: Args: {args}")
        ...
        try:
            if not self.tools or 'vector_ingestion' not in self.tools:
                # Simulate vector ingestion for testing purposes
                print(f"⚠️ DEBUG: Vector ingestion tool not available, simulating process...")
                ...
                result = {
                    ...
                    "metadata": metadata,
                    "simulation": True  # Indicates this is a simulated result
                }
                
                print(f"✅ DEBUG: Vector ingestion simulation completed: {result}")
                self.logger.info(f"Vector ingestion simulated for {doc_uri}")
                return result
            
            vector_tool = self.tools['vector_ingestion']
            print(f"🔧 DEBUG: Vector tool retrieved: {type(vector_tool)}")
            print(f"🔧 DEBUG: Vector tool methods: {dir(vector_tool)}")
            
            print(f"🔧 DEBUG: Calling vector_tool.ingest with content length {len(content)}")
            result = await vector_tool.ingest(content, metadata)
            print(f"✅ DEBUG: Vector ingestion completed successfully: {result}")
            
            self.logger.info(f"Vector ingestion completed for {doc_uri}")
            return result
            
        except Exception as e:
            print(f"❌ DEBUG: Vector ingestion failed: {e}")
            import traceback
            traceback.print_exc()
            raise e
    ...
    async def _execute_graph_ingestion_with_content(self, doc_uri: str, content: str, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            if not self.tools or 'graph_ingestion' not in self.tools:
                # Simulate graph ingestion for testing purposes
                print(f"⚠️ DEBUG: Graph ingestion tool not available, simulating process...")
                
                # Simulate graph creation
                nodes_created = min(10, len(content) // 100 + 1)  # Simulate some nodes
                relationships_created = max(0, nodes_created - 1)  # Simulate relationships
                ...
                print(f"✅ DEBUG: Graph ingestion simulation completed: {result}")
                self.logger.info(f"Graph ingestion simulated for {doc_uri}")
                return result
            ...
            print(f"❌ DEBUG: Graph ingestion failed: {e}")
            import traceback
            traceback.print_exc()
            raise e
